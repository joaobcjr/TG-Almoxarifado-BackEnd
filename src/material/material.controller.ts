...
import { GetMaterialDto, InsertMaterialDto, UpdateMaterialDto } from './material.dto';
...
export class MaterialController {
    ...
    @Get()
    getMaterial(@Query(ValidationPipe) getMaterialDto: GetMaterialDto): Promise<Material[]> {
        return this.materialService.getMaterial(getMaterialDto);
    }

    @Post()
    async insertMaterial(@Body(ValidationPipe) insertMaterialDto: InsertMaterialDto): Promise<Material> {
        return this.materialService.insertMaterial(insertMaterialDto);
    }

    @Patch()
    async patchMaterial(@Body() updateMaterialDto: UpdateMaterialDto): Promise<Material> {
