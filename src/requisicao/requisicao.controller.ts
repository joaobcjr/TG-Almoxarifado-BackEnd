...
export class RequisicaoController {
  ...
  @Get()
  getRequisicao(
    @Query(ValidationPipe) getRequisicaoDto: GetRequisicaoDto,
  ): Promise<Requisicao[]> {
  ...
  async insertRequisicao(
    @Body(ValidationPipe) insertRequisicaoDto: InsertRequisicaoDto,
  ): Promise<Requisicao> {
  ...
  async patchRequisicao(
    ...
    return await this.requisicaoService.atenderRequisicao(
      id,
      requisicaoAtenderDto,
    );
  }
