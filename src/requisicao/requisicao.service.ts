...
export class RequisicaoService {
  ...
  async insertRequisicao(
    ...
    for (let x = 0; x < insertRequisicaoDto.materiais.length; x++) {
      await this.materialService.getMaterialById(
        insertRequisicaoDto.materiais[x].id_material,
      );
    }
    return await this.requisicaoRepository.insertRequisicao(
      insertRequisicaoDto,
    );
  }

  async getRequisicaoById(id: number): Promise<Requisicao> {
    const found = await this.requisicaoRepository.getRequisicaoById(id);
  ...
  async getRequisicao(
    ...
    return await this.requisicaoRepository.getFilteredRequisicao(
      getRequisicaoDto,
    );
  }
  ...
  async atenderRequisicao(
    ...
    return await this.requisicaoRepository.atenderRequisicao(
      requisicaoAtenderDto,
      requisicao,
    );
  }
