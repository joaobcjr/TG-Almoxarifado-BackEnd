...
export class SolicitacaoService {
    ...
    async insertSolicitacao(insertSolicitacaoDto: InsertSolicitacaoDto): Promise<Solicitacao> {
        for (let x = 0; x < insertSolicitacaoDto.materiais.length; x++) {
            await this.materialService.getMaterialById(insertSolicitacaoDto.materiais[x].id_material);
        }
        return await this.solicitacaoRepository.insertSolicitacao(insertSolicitacaoDto);
    }

    async getSolicitacaoById(id: number): Promise<Solicitacao> {

        const found = await this.solicitacaoRepository.findOne({
            where: {
                id_solicitacao: id
            },
            relations: ['solicitacao_material', 'funcionario']
        });
    ...
    async getSolicitacao(getSolicitacaoDto: GetSolicitacaoDto): Promise<Solicitacao[]> {
        return await this.solicitacaoRepository.getFilteredSolicitacao(getSolicitacaoDto);
    }
}
